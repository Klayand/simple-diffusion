...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, max_seq_len: int, dim: int):
        ...
        # Assume dim is an even number for convenience
        assert dim % 2 == 0
        ...
        pe_2i = torch.sin(pos / 10000 ** (two_i / dim))
        pe_2i_1 = torch.cos(pos / 10000 ** (two_i / dim))
        pe = torch.stack((pe_2i, pe_2i_1), 2).reshape(max_seq_len, dim)
...
class ConvNet(nn.Module):
    ...
    def __init__(self,
                 ...
                 pe_dim = 10,
                 insert_t_to_all_layer=False):
        super(ConvNet, self).__init__()

        C, H, W = (1, 28, 28)
        self.pe = PositionalEncoding(n_steps, pe_dim)
        self.pe_linears = nn.ModuleList()
        self.all_t = insert_t_to_all_layer

        if not insert_t_to_all_layer:
...
class UnetBlock(nn.Module):
    ...
    def __init__(self, shape, in_dim, out_dim, residual=False):
        ...
        if residual:
            if in_dim == out_dim:
                ...
                self.residual_conv = nn.Conv2d(in_dim, out_dim, 1)
